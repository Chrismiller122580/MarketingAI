...
export const INTEGRATION_GUIDES: IntegrationGuide[] = [
  ...
  },
  {
    ...
    steps: [
      "Create a Meta app at developers.facebook.com → add Facebook Login + Pages API (and Instagram if you publish Reels).",
      "Facebook Login → Settings → Valid OAuth Redirect URIs: https://www.crawlspark.ai/api/auth/callback/facebook and https://crawlspark.ai/api/auth/callback/facebook.",
      "Do not use Facebook Login for Business / FACEBOOK_LOGIN_CONFIG_ID unless you explicitly need ads. crawlspark publishes to Pages with standard Facebook Login.",
      "Option A (global): generate a long-lived Page access token + set FACEBOOK_PAGE_ACCESS_TOKEN and FACEBOOK_PAGE_ID.",
      "Option B (per-site): set FACEBOOK_CLIENT_ID and FACEBOOK_CLIENT_SECRET, then use Connect Facebook on Settings.",
      "If the Meta app is in Development, add the Facebook account as Admin, Developer, or Tester.",
